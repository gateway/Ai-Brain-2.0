...
export interface TemporalTruthDecayScenarioResult {
  readonly id: string;
  readonly query: string;
...
export interface TemporalTruthDecayPackReport {
  readonly generatedAt: string;
  readonly benchmark: "temporal_truth_decay_pack";
  readonly runtime: BenchmarkRuntimeMetadata;
  readonly sampleCount: number;
  readonly passed: boolean;
  readonly results: readonly TemporalTruthDecayScenarioResult[];
}
...
function generatedRoot(): string {
  return path.resolve(localBrainRoot(), "benchmark-generated", "temporal-truth-decay-pack");
}

function fixtures(): readonly FixtureFile[] {
  return [
    {
      relativePath: "2024-01-02-coffee-history.md",
      sourceType: "markdown",
      capturedAt: "2024-01-02T09:00:00.000Z",
      body: ["# Coffee preference", "", "I prefer espresso coffee."].join("\n")
    },
    {
      relativePath: "2026-01-02-coffee-change.md",
      sourceType: "markdown",
      capturedAt: "2026-01-02T09:00:00.000Z",
      body: [
        "# Coffee change",
        "",
        "I do not like espresso coffee anymore because it hurts my stomach.",
...
async function ingestFixtures(namespaceId: string, rootPath: string, files: readonly FixtureFile[]): Promise<void> {
  for (const fixture of files) {
    await ingestArtifact({
      ...
      sourceChannel: "benchmark:temporal_truth_decay_pack",
      capturedAt: fixture.capturedAt,
      skipNarrativeClaims: true,
      metadata: {
        benchmark: "temporal_truth_decay_pack",
  ...
  return [
    {
      ...
      query: "What do I prefer now for coffee?",
      expectedTerms: ["pour-over coffee"]
    },
    {
      id: "historical_coffee_preference",
      query: "What did I use to prefer for coffee?",
      expectedTerms: ["espresso coffee"],
      forbiddenTerms: ["pour-over coffee"]
    },
    {
      id: "point_in_time_coffee_preference",
      query: "What did I prefer in 2024 for coffee?",
      expectedTerms: ["espresso coffee"],
      forbiddenTerms: ["pour-over coffee"]
    }
  ];
...
async function runScenario(namespaceId: string, scenario: Scenario): Promise<TemporalTruthDecayScenarioResult> {
...
function toMarkdown(report: TemporalTruthDecayPackReport): string {
  const lines = [
    "# Temporal Truth Decay Pack",
    "",
...
export async function runAndWriteTemporalTruthDecayPack(): Promise<{
  readonly report: TemporalTruthDecayPackReport;
  readonly output: { readonly jsonPath: string; readonly markdownPath: string };
}> {
  const stamp = new Date().toISOString().replace(/[:.]/g, "-");
  const namespaceId = `benchmark_temporal_truth_decay_pack_${stamp}`;
  ...
  const results: TemporalTruthDecayScenarioResult[] = [];
  for (const scenario of scenarios()) {
    results.push(await runScenario(namespaceId, scenario));
  }
  const report: TemporalTruthDecayPackReport = {
  ...
  const jsonPath = path.join(outputDir(), `temporal-truth-decay-pack-${stampName}.json`);
  const markdownPath = path.join(outputDir(), `temporal-truth-decay-pack-${stampName}.md`);
  await writeFile(jsonPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");
  await writeFile(markdownPath, toMarkdown(report), "utf8");
  return { report, output: { jsonPath, markdownPath } };
}

export async function runTemporalTruthDecayPackCli(): Promise<void> {
  try {
    const { report, output } = await runAndWriteTemporalTruthDecayPack();
