    ...
    for (const [sampleId, sampleScenarios] of scenarioGroups.entries()) {
      ...
      for (const [sessionKey, turns] of sessionEntries) {
        const ingestResult = await ingestLoCoMoSessionArtifacts({
          ...
          benchmarkName: "full_standard_residual_review",
        ...
        if (ingestResult.imageArtifactCount > 0) {
          console.log(
            `[full-standard-residual-review] sample=${sample.sample_id} session=${sessionKey} imageArtifacts=${ingestResult.imageArtifactCount} imageDerivations=${ingestResult.derivedImageCount} cacheHits=${ingestResult.imageDerivationCacheHits}`
          );
        }
      }
