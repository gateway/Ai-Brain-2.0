...
type ToolName = "memory.recap" | "memory.explain_recap";
...
export interface RecapFamilyBenchmarkReport {
...
function scenarios(namespaceId: string): readonly Scenario[] {
  return [
    {
      name: "participant_yesterday",
      tool: "memory.recap",
      args: {
        namespace_id: namespaceId,
        query: "Can you summarize what Steve and Dan talked about yesterday?",
        reference_now: "2026-03-21T12:00:00Z",
        limit: 8
      },
      expectedTerms: ["Dan", "Project A"],
      expectedConfidence: "confident",
      minimumEvidence: 1
    },
    {
      name: "participant_topic_yesterday",
      tool: "memory.recap",
      args: {
        namespace_id: namespaceId,
    ...
    {
      name: "topic_yesterday",
      tool: "memory.recap",
      args: {
        namespace_id: namespaceId,
        query: "Give me a recap of what we said about Project A yesterday.",
        reference_now: "2026-03-21T12:00:00Z",
        limit: 8
      },
      expectedTerms: ["Project A", "retrieval planner"],
      expectedConfidence: "confident",
      minimumEvidence: 1
    },
    {
      name: "project_yesterday",
      tool: "memory.recap",
      args: {
        namespace_id: namespaceId,
        query: "What came up for Project A yesterday?",
        reference_now: "2026-03-21T12:00:00Z",
        limit: 8
      },
      expectedTerms: ["Project A", "demo outline"],
      expectedConfidence: "confident",
      minimumEvidence: 1
    },
    {
      name: "last_weekend_people",
      tool: "memory.recap",
      args: {
        namespace_id: namespaceId,
      ...
      expectedTerms: ["Jules", "Rina", "karaoke"],
      expectedConfidence: "confident",
      minimumEvidence: 1
    },
    {
      name: "why_evidence_recap",
      tool: "memory.explain_recap",
      args: {
        namespace_id: namespaceId,
        query: "Why do you think that was the right conversation about Project A yesterday?",
        reference_now: "2026-03-21T12:00:00Z",
        limit: 8
      },
      expectedTerms: ["Project A", "grouped"],
      expectedConfidence: "confident",
      minimumEvidence: 1
    }
  ];
...
async function runScenario(scenario: Scenario): Promise<ScenarioResult> {
  ...
  if (sourceLinkCount(evidence) < scenario.minimumEvidence) {
    failures.push("expected source links for recap evidence");
  }
...
function toMarkdown(report: RecapFamilyBenchmarkReport): string {
  const lines = [
  ...
  for (const result of report.results) {
    lines.push(
      `- ${result.name}: ${result.passed ? "pass" : "fail"} | tool=${result.tool} | confidence=${result.confidence ?? "n/a"} | evidence=${result.evidenceCount} | sourceLinks=${result.sourceLinkCount} | latencyMs=${result.latencyMs}`
    );
...
export async function runAndWriteRecapFamilyBenchmark(): Promise<{
  readonly report: RecapFamilyBenchmarkReport;
  readonly output: {
  ...
  const report: RecapFamilyBenchmarkReport = {
  ...
  const jsonPath = path.join(outDir, `recap-family-${stamp}.json`);
  const markdownPath = path.join(outDir, `recap-family-${stamp}.md`);
...
export async function runRecapFamilyBenchmarkCli(): Promise<void> {
  const { output, report } = await runAndWriteRecapFamilyBenchmark();
