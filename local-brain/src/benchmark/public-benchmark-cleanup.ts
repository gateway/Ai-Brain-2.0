...
const DEFAULT_CLEANUP_STAGE_ATTEMPTS = 4;
type NamespacePrefixInput = string | readonly string[];
...
function cleanupErrorLabel(error: unknown): string {
...
export async function cleanupPublicBenchmarkNamespaces(
  ...
  const stageAttempts = Math.max(1, Math.floor(options?.stageAttempts ?? DEFAULT_CLEANUP_STAGE_ATTEMPTS));
  const logger = options?.logger ?? (() => {});

  const configureCleanupSession = async (client: Pick<PoolClient, "query">, attempt: number): Promise<void> => {
    const effectiveLockTimeoutMs = Math.min(30_000, lockTimeoutMs * attempt);
    await client.query(`SET LOCAL timescaledb.max_tuples_decompressed_per_dml_transaction = 0`);
    await client.query(`SET LOCAL statement_timeout = ${statementTimeoutMs}`);
    await client.query(`SET LOCAL lock_timeout = ${effectiveLockTimeoutMs}`);
  };
  ...
  const runCleanupStage = async (
    ...
    for (let attempt = 1; attempt <= stageAttempts; attempt += 1) {
      logger(`cleanup stage start ${label} attempt=${attempt}/${stageAttempts}`);
      try {
        await withTransaction(async (client) => {
          await configureCleanupSession(client, attempt);
          await fn(client);
        });
        logger(`cleanup stage complete ${label} attempt=${attempt}/${stageAttempts}`);
        return;
      } catch (error) {
        if (!transientCleanupError(error) || attempt >= stageAttempts) {
          throw error;
        }
        const delayMs = Math.min(5_000, 250 * 2 ** (attempt - 1));
        logger(`cleanup stage retry ${label} attempt=${attempt}/${stageAttempts} delayMs=${delayMs} error=${cleanupErrorLabel(error)}`);
        await sleep(delayMs);
      }
    }
  };
  ...
  for (const namespaceChunk of namespaceChunks) {
    ...
    await runCleanupStage(`namespace scoped tables namespaces=${namespaceChunk.join(",")}`, async (client) => {
      await client.query(`DELETE FROM compiled_memory_coverage WHERE namespace_id = ANY($1::text[])`, [namespaceChunk]);
      await client.query(`DELETE FROM compiled_relationship_observations WHERE namespace_id = ANY($1::text[])`, [namespaceChunk]);
      await client.query(`DELETE FROM compiled_event_observations WHERE namespace_id = ANY($1::text[])`, [namespaceChunk]);
      await client.query(`DELETE FROM compiled_fact_observations WHERE namespace_id = ANY($1::text[])`, [namespaceChunk]);
      await client.query(`DELETE FROM exact_detail_fact_keys WHERE namespace_id = ANY($1::text[])`, [namespaceChunk]);
      await client.query(`DELETE FROM temporal_event_support WHERE namespace_id = ANY($1::text[])`, [namespaceChunk]);
      await client.query(`DELETE FROM temporal_event_facts WHERE namespace_id = ANY($1::text[])`, [namespaceChunk]);
      await client.query(`DELETE FROM contract_projection_entries WHERE namespace_id = ANY($1::text[])`, [namespaceChunk]);
      await client.query(`DELETE FROM contract_projection_heads WHERE namespace_id = ANY($1::text[])`, [namespaceChunk]);
...
export async function listResidualBenchmarkNamespaces(prefix: NamespacePrefixInput = "benchmark_"): Promise<readonly string[]> {
  ...
  const rows = await queryRows<{ readonly namespace_id: string }>(
    `
      WITH namespaces AS (
        SELECT namespace_id
        FROM artifacts
        WHERE namespace_id LIKE ANY($1::text[])
        UNION
        SELECT namespace_id
    ...
    `,
    [normalizedPrefixes.map((value) => `${value}%`)]
  );
...
export async function scrubResidualBenchmarkNamespaces(
  prefix: NamespacePrefixInput = "benchmark_",
  options?: {
    readonly namespaceChunkSize?: number;
    readonly statementTimeoutMs?: number;
    readonly lockTimeoutMs?: number;
  ...
  const prefixLabel = typeof prefix === "string" ? prefix : prefix.join(",");
  return {
    prefix: prefixLabel,
