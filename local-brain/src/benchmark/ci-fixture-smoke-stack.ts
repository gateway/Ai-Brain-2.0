...
interface CiFixtureSmokeStepResult extends CiFixtureSmokeStep {
...
export interface CiFixtureSmokeStackReport {
  readonly generatedAt: string;
  readonly benchmark: "ci_fixture_smoke_stack";
  readonly artifactSchemaVersion: "ci_fixture_smoke_stack_v1";
  readonly passed: boolean;
  readonly metrics: {
  ...
  readonly steps: readonly CiFixtureSmokeStepResult[];
}

const STACK_STEPS: readonly CiFixtureSmokeStep[] = [
  {
    id: "benchmark_reliability_audit",
    scriptName: "benchmark:benchmark-reliability-audit",
    reason: "Proves benchmark scripts are registered or explicitly legacy-covered before trusting CI product gates."
  },
  {
    id: "mcp_query_taxonomy_gold",
    scriptName: "benchmark:mcp-query-taxonomy-gold",
    reason: "Primary MCP query contract, routing, answer-shape, evidence, source-trail, and no-query-time-model gate.",
    env: {
  ...
  },
  {
    ...
    reason: "Fixture-level source-audit envelope and claim-audit coverage across answer families."
  },
  {
    id: "temporal_memory_query_audit",
    scriptName: "benchmark:temporal-memory-query-audit",
    reason: "Temporal source scope, event-window scope, fuzzy time, and lifecycle-scope retrieval gate."
  },
  {
...
async function runStep(step: CiFixtureSmokeStep): Promise<CiFixtureSmokeStepResult> {
...
function summarize(results: readonly CiFixtureSmokeStepResult[]): CiFixtureSmokeStackReport["metrics"] {
...
function toMarkdown(report: CiFixtureSmokeStackReport): string {
  const lines = [
    "# CI Fixture Smoke Stack",
    "",
...
export async function runCiFixtureSmokeStack(): Promise<CiFixtureSmokeStackReport> {
  const results: CiFixtureSmokeStepResult[] = [];
  for (const step of STACK_STEPS) {
    console.log(`\n[ci-fixture-smoke-stack] running ${step.scriptName}`);
    const result = await runStep(step);
    results.push(result);
    if (!result.passed) break;
  }
  const report: CiFixtureSmokeStackReport = {
    generatedAt: new Date().toISOString(),
    benchmark: "ci_fixture_smoke_stack",
  ...
  const jsonPath = path.join(outputDir(), `ci-fixture-smoke-stack-${stamp}.json`);
  const mdPath = path.join(outputDir(), `ci-fixture-smoke-stack-${stamp}.md`);
...
export async function runCiFixtureSmokeStackCli(): Promise<void> {
  const report = await runCiFixtureSmokeStack();
