  ...
  try {
    ...
    for (const [sessionKey, turns] of sessionEntries) {
      const ingestResult = await ingestLoCoMoSessionArtifacts({
        ...
        benchmarkName: "full_standard_pressure_replay",
      ...
      if (ingestResult.imageArtifactCount > 0) {
        console.log(
          `[full-standard-pressure-replay] sample=${sample.sample_id} session=${sessionKey} imageArtifacts=${ingestResult.imageArtifactCount} imageDerivations=${ingestResult.derivedImageCount} cacheHits=${ingestResult.imageDerivationCacheHits}`
        );
      }
    }
