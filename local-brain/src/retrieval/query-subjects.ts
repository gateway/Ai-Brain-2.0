...
export function isDependentGroupScopeQuery(queryText: string): boolean {
  return /\bwhat\s+do\b[^?!.]{0,60}\b(?:kids?|children|sons?|daughters?)\b[^?!.]{0,20}\blike\b/iu.test(queryText);
}

export function inferSubjectBoundAggregationScope(queryText: string, pairRequired: boolean): "primary_subject" | "dependent_group" | "pair_subject" {
  if (pairRequired) {
    return "pair_subject";
  }
  return isDependentGroupScopeQuery(queryText) ? "dependent_group" : "primary_subject";
}
...
export function extractPrimaryQuerySurfaceNames(queryText: string): readonly string[] {
  const patterns = [
    ...
    /\bWhere\s+(?:has|had|did|does|was|is)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})(?:'s)?\b/u,
