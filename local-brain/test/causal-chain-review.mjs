...
test("realization derivation abstains when the same speaker only reacts without an explicit realization cue", () => {
  const claim = deriveRealizationClaimText("What did Caroline realize after her charity race?", [
    makeResult("Caroline: It was nice hearing everyone cheer me on after the charity race.", {
      memoryId: "caroline-anchor",
      metadata: {
        source_turn_text: "Caroline: It was nice hearing everyone cheer me on after the charity race."
    ...
    makeResult("Caroline: Taking care of ourselves is so important - even if it's not always easy.", {
      memoryId: "caroline-reflection",
      metadata: {
        source_turn_text: "Caroline: Taking care of ourselves is so important - even if it's not always easy."
      }
    })
  ]);
...
test("realization derivation with explicit entity and possessive anchor does not fall back to another speaker", () => {
  const claim = deriveRealizationClaimText("What did Caroline realize after her charity race?", [
    makeResult("Caroline: It was nice hearing everyone cheer me on after the charity race.", {
      memoryId: "caroline-anchor-her",
      metadata: {
        source_turn_text: "Caroline: It was nice hearing everyone cheer me on after the charity race."
      }
    }),
    makeResult(
      "Melanie: I ran a charity race for mental health last Saturday – it was really rewarding. The event was really thought-provoking. I'm starting to realize that self-care is really important.",
      {
        memoryId: "melanie-realization-her",
        metadata: {
          source_turn_text:
            "Melanie: I ran a charity race for mental health last Saturday – it was really rewarding. The event was really thought-provoking. I'm starting to realize that self-care is really important."
        }
...
test("realization derivation does not pair an anchored turn with a later unrelated same-speaker realization", () => {
  const claim = deriveRealizationClaimText("What did Caroline realize after her charity race?", [
    makeResult("Caroline: It was nice hearing everyone cheer me on after the charity race.", {
      memoryId: "caroline-charity-anchor",
      metadata: {
        source_turn_text:
          "Host: How did the charity race feel? Caroline: It was nice hearing everyone cheer me on after the charity race. Caroline: I'm starting to realize that living authentically matters."
      }
    })
  ]);
...
test("realization derivation only accepts split-turn continuations when they bridge back to the anchor event", () => {
  const claim = deriveRealizationClaimText("What did Caroline realize after her charity race?", [
    makeResult("Caroline: It was nice hearing everyone cheer me on after the charity race.", {
      memoryId: "caroline-charity-anchor-bridge",
      metadata: {
        source_turn_text:
          "Caroline: It was nice hearing everyone cheer me on after the charity race. Caroline: I'm starting to realize that I can be me without fear and having the courage to transition was the best part."
      }
    })
  ]);
...
test("financial status derivation ignores cross-subject fallback when primary-subject evidence is present", () => {
  const claim = deriveCausalMotiveClaimText(
    "What might John's financial status be?",
    [
      makeResult(
      ...
      makeResult("John: I've done weight training so far too. It was challenging but peaceful, kinda like yoga.", {
        memoryId: "john-noise",
        metadata: {
          source_turn_text:
...
test("civic motive derivation ignores generic neighboring chatter when a formative trigger is present", () => {
  const claim = deriveCausalMotiveClaimText(
    "What sparked John's interest in improving education and infrastructure in the community?",
    [
      makeResult(
  ...
  assert.equal(claim, "lack of education and crumbling infrastructure affected my neighborhood");
});
